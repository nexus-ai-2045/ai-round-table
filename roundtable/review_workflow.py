...
def _changed_files(repo: Path, base: str, commit: str) -> tuple[list[str] | None, str]:
    """base..commit の変更 path を取る。
...
def validate_live_git(data: dict, *, phase: str, repo: Path) -> list[str]:
    ...
    if phase == "start":
        ...
        changed, detail = _changed_files(repo, data["base_commit"], commit_sha)
        if changed is None:
            errors.append(f"fan_in: changed files を取得できない: {detail}")
        else:
            owned = set(data["implementation"]["owned_files"])
            outside = sorted(set(changed) - owned)
            if outside:
