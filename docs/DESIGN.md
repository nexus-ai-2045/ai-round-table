...
- v5→v6: 追加調査・実測の統合 (機能変更なし):
  ...
  [docs/prior-art.md](prior-art.md)（2026-09-01 に公開できる形で置き直した。
  元は非公開の内部調査だったため、公開 repo から辿れなかった）
...
| # | 決定 | 根拠 |
|---|---|---|
| D1 | 司会 = 人間。裁定・打ち切り・指名・メンバー変更は CEO 宣言のみ | 席間の直接通信と dispatcher による要約を作らず、共有議事録を読む blackboard 構造 + CC#1。旧根拠「網目 17.2x / 中央 4.4x (arXiv 2512.08296)」は **2026-09-01 に撤回** (review-backlog 参照)。決定自体は維持 |
