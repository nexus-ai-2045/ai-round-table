...
```bash
python3 -m roundtable.cli new-topic TOPIC --topic '議題' --participants cc --root ROOT
python3 -m roundtable.cli dispatch TOPIC --participant cc --no-clipboard --async --root ROOT
```
...
```bash
python3 -m roundtable.cli handoff TOPIC --invocation INV --transport claude-desktop --root ROOT
python3 -m roundtable.cli handoff TOPIC --invocation INV --transport claude-desktop --apply --root ROOT
```
...
```bash
python3 -m roundtable.cli handoff TOPIC --invocation INV --transport cmux \
...
```bash
python3 -m roundtable.cli handoff-status TOPIC --invocation INV --root ROOT
python3 -m roundtable.cli collect-pending TOPIC --timeout 30 --root ROOT
python3 -m roundtable.cli cancel TOPIC --invocation INV --root ROOT
```
